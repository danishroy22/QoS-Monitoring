...
- `X-SmartQoS-ISP`: required for ISP admin when auth is enforced
...
When `QOS_AUTH_REQUIRED=false` (default), local runs work without tokens.
...
`QOS_AUTH_REQUIRED=true`. Consumer speed-test UI is unchanged.
